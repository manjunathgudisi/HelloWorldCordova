...
function NewDcFacade($scope, $location, xlatService, toaster, SpinService) {
    ...
    /// </summary>
    this.PageLoadOLD = function () {
        try {
            OneViewConsole.Debug("PageLoad Start", "NewDcFacade.PageLoad");

            var CurrentShift = new LVShiftHandler().GetCurrentShift();

           // alert('CurrentShift : ' + JSON.stringify(CurrentShift));
          
            //TemplateNodeId,DcPlaceId,ServerId,ReccurenceId,Occurence,DcProfileId ,(and some other columns too)
            var DcProfileList = oDCProfileDAOObj.GetDcProfile(ServiceId, UserId, -1, CurrentShift.Id,-1,-1);                     
            GlobalDcProfileList = DcProfileList;

         //   alert('GlobalDcProfileList : ' + GlobalDcProfileList.length + "," + JSON.stringify(GlobalDcProfileList));

            var TemporaryGlobalDcProfileList = clone(GlobalDcProfileList);
            //Get DcProfiles by occurence
            for (var i = 0; i < TemporaryGlobalDcProfileList.length ; i++) {
                oMyInstance.UpdateDcProfileByOccurence(TemporaryGlobalDcProfileList[i].DcPlaceId, TemporaryGlobalDcProfileList[i].TemplateNodeId, CurrentShift, TemporaryGlobalDcProfileList[i].CustomPlaceName);
            }
           ...
           // var FaciltyData = oDCProfileDAOObj.GetDCPlaceDimension(ServiceId, UserId, -1, -1, -1, -1);
            var FaciltyData = oDCProfileDAOObj.GetDCPlaceDimension(DcPlaceIdDetailsList);

          //  alert("PageLoadDCPlaceDimensionlst : " + JSON.stringify(FaciltyData));


            //for (var i = 0; i < DcPlaceIdDetailsList.length; i++) {
            //    if (DcPlaceIdDetailsList[i].DcPlaceId == 0 && DcPlaceIdDetailsList[i].CustomPlaceName != '') {

            //        FaciltyData.push({ 'ChildDbElementType': -999 });
            //        break;
            //    }
            //}
           ...
           // alert("PageLoadDCPlaceDimensionlst : " + JSON.stringify(FaciltyData));
            ...
            for (var i = 0; i < FaciltyData.length; i++)
            {
                var DcPlaceDimension = FaciltyData[i]['ChildDbElementType'];
               // alert('DcPlaceDimension :' + DcPlaceDimension);
                if (DcPlaceDimension == DATEntityType.RCOMaster_Kitchen) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_Kitchen, Name: "Unit", ControlId: "chkUnit", 'ColourIndex': 'green' })
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_Supplier) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_Supplier, Name: "Supplier", ControlId: "chkSupplier", 'ColourIndex': 'green' })
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_FandBOutLet) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_FandBOutLet, Name: "FandBOutLet", ControlId: "chkFandBOutLet", 'ColourIndex': 'green' })
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_Location) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_Location, Name: "Section", ControlId: "chkSection", 'ColourIndex': 'green' });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_DIA_Non_DIA) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_DIA_Non_DIA, Name: "Dia and Non Dia F & B", ControlId: "chkDiaNonDia", 'ColourIndex': 'green' });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_AccomodationLocation) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_AccomodationLocation, Name: "Accomodation Location", ControlId: "chkAccomodationLocation", 'ColourIndex': 'green' });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_SamplingPoint) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_SamplingPoint, Name: "Sampling Point", ControlId: "chkSamplingUnit", 'ColourIndex': 'green' });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_Section) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_Section, Name: "Location", ControlId: "chkLocation", 'ColourIndex': 'green' });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_Department) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_Department, Name: "Department", ControlId: "chkDepartment", 'ColourIndex': 'green' });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_Flight) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_Flight, Name: "Flight", ControlId: "chkFlight", 'ColourIndex': 'green' });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_CleaningType) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_CleaningType, Name: "Cleaning Type", ControlId: "chkCleaningType", 'ColourIndex': 'green' });
                }
                //if (DcPlaceDimension == -999) {
                //    $scope.DcPlaceDimensions.push({ Id: -999, Name: "Sampling Unit", ControlId: "chkSamplingUnit", 'ColourIndex': 'green' });
                //}
            }

            //alert('$scope.DcPlaceDimensions :' + $scope.DcPlaceDimensions.length)
            //alert('$scope.DcPlaceDimensions 222 :' + JSON.stringify($scope.DcPlaceDimensions));
            // ochkDcPlaceDimensions.Set({ Id: DATEntityType.RCOMaster_Supplier, Name: "Supplier", ControlId: "chkSupplier", 'ColourIndex': 'green' });
    ...
    this.PageLoad = function () {
        try {
            ...
            for (var i = 0; i < FaciltyData.length; i++) {
                ...
                if (DcPlaceDimension == DATEntityType.RCOMaster_Kitchen) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_Kitchen, Name: "Unit", ControlId: "chkUnit", 'ColourIndex': 'green', Selected: SelectedValue })
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_Supplier) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_Supplier, Name: "Supplier", ControlId: "chkSupplier", 'ColourIndex': 'green', Selected: SelectedValue })
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_FandBOutLet) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_FandBOutLet, Name: "FandBOutLet", ControlId: "chkFandBOutLet", 'ColourIndex': 'green', Selected: SelectedValue })
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_Location) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_Location, Name: "Section", ControlId: "chkSection", 'ColourIndex': 'green', Selected: SelectedValue });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_DIA_Non_DIA) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_DIA_Non_DIA, Name: "Dia and Non Dia F & B", ControlId: "chkDiaNonDia", 'ColourIndex': 'green', Selected: SelectedValue });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_AccomodationLocation) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_AccomodationLocation, Name: "Accomodation Location", ControlId: "chkAccomodationLocation", 'ColourIndex': 'green', Selected: SelectedValue });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_SamplingPoint) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_SamplingPoint, Name: "Sampling Point", ControlId: "chkSamplingUnit", 'ColourIndex': 'green', Selected: SelectedValue });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_Section) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_Section, Name: "Location", ControlId: "chkLocation", 'ColourIndex': 'green', Selected: SelectedValue });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_Department) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_Department, Name: "Department", ControlId: "chkDepartment", 'ColourIndex': 'green', Selected: SelectedValue });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_Flight) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_Flight, Name: "Flight", ControlId: "chkFlight", 'ColourIndex': 'green', Selected: SelectedValue });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_CleaningType) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_CleaningType, Name: "Cleaning Type", ControlId: "chkCleaningType", 'ColourIndex': 'green', Selected: SelectedValue });
                }
                if (DcPlaceDimension == DATEntityType.RCOMaster_ChemicalSupplier) {
                    $scope.DcPlaceDimensions.push({ Id: DATEntityType.RCOMaster_ChemicalSupplier, Name: "Chemical Supplier", ControlId: "chkChemicalSupplier", 'ColourIndex': 'green', Selected: SelectedValue });
                }                
                //if (DcPlaceDimension == -999) {
                //    $scope.DcPlaceDimensions.push({ Id: -999, Name: "Sampling Unit", ControlId: "chkSamplingUnit", 'ColourIndex': 'green' });
                //}
    ...
    this.LoadDDLOLD = function (DcPlaceDimensions,RCOType)
    {
        var FaciltyData = oDCProfileDAOObj.GetDCPlace(ServiceId, UserId, -1, -1, -1, -1, DcPlaceDimensions, RCOType);
        oNewDcPresenter.Facility(FaciltyData);



        var TemplateData = oDCProfileDAOObj.GetAuditTemplate(ServiceId, UserId, -1, -1, -1, -1, $scope.Facility.Id);
        NewDcPlaceWiseTemplateList = oNewDcPresenter.Template(TemplateData);
        //alert('PageLoad NewDcPlaceWiseTemplateList  ' + JSON.stringify(NewDcPlaceWiseTemplateList));
    ...
    this.LoadDDL22 = function (DcPlaceDimensions, RCOType, DcPlaceIdDetailsList) {
        try {
            OneViewConsole.Debug("LoadDDL Start", "NewDcFacade.LoadDDL");
            var CurrentShift = new LVShiftHandler().GetCurrentShift();

            if (DcPlaceIdDetailsList == undefined || DcPlaceIdDetailsList == '' || DcPlaceIdDetailsList == null) {
                DcPlaceIdDetailsList = oMyInstance.GetDcPlaces();
            }


            var FaciltyData = oDCProfileDAOObj.GetDCPlace(DcPlaceIdDetailsList, RCOType);
            oNewDcPresenter.Facility(FaciltyData);

           // alert('$scope.Facility.Id :' + $scope.Facility.Id);
            ...
            //alert('oProfileCountBefore :' + oProfileCountBefore);
           // alert($scope.Facility.Id + ", Template: " + $scope.Template.Id);
            ...
            if (oProfileCountBefore > GlobalDcProfileList.length) {
               // alert('oProfileCountBefore :' + oProfileCountBefore + ' ,     GlobalDcProfileList.length' + GlobalDcProfileList.length);
                DcPlaceIdDetailsList = oMyInstance.GetDcPlaces();
              //  alert('DcPlaceIdDetailsList : ' + JSON.stringify(DcPlaceIdDetailsList));

                if (DcPlaceIdDetailsList.length > 0) {
                
                    FaciltyData = oDCProfileDAOObj.GetDCPlace(DcPlaceIdDetailsList, RCOType);

                  //  alert('FaciltyData : ' + JSON.stringify(FaciltyData));
                    oNewDcPresenter.Facility(FaciltyData);

                  //  alert('Facility : ' + $scope.Facility.Id);
                
                    if ($scope.Facility != undefined) {
                        TemplateData = oDCProfileDAOObj.GetAuditTemplate($scope.Facility.Id, $scope.Facility.Name);
                        // alert('TemplateData : ' + JSON.stringify(TemplateData));
                        NewDcPlaceWiseTemplateList = oNewDcPresenter.Template(TemplateData);

                        //  alert('Template : ' + $scope.Template.Id);
                        TemplateGroupList = oDCProfileDAOObj.GetAuditTemplateGroup(TemplateData);
                        oNewDcPresenter.TemplateGroup(TemplateGroupList);

                        if ($scope.TemplateGroup != undefined) {
                            TemplateList = oDCProfileDAOObj.GetAuditTemplateBasedOnTemplateGroup($scope.TemplateGroup.Id);
                            oNewDcPresenter.Template(TemplateList);
    ...
    this.LoadDDL = function (DcPlaceDimensions, RCOType, DcPlaceIdDetailsList) {
        try {
            OneViewConsole.Debug("LoadDDL Start", "NewDcFacade.LoadDDL");
            var CurrentShift = new LVShiftHandler().GetCurrentShift();
    ...
    this.SelectedChangeOLD = function () {
        try {
    ...
    this.TemplateGroupSelectedChangeOLD = function (TemplateGroupId) {
        try {
            OneViewConsole.Debug("TemplateGroupSelectedChange Start", "NewDcFacade.TemplateGroupSelectedChange");

           // $scope.TemplateList
            //oNewDcPresenter.Template(TemplateData);
    ...
    this.TemplateSelectedChange = function () {
        try {
...
function DefaultDCProfileDAO() {
    try {
        ...
        this.GetDCPlaceOLD = function (ServiceId, UserId, TemplateNodeId, DCDate, FromTime, ToTime, DcPlaceDimension, RCOType) {
            try {
                OneViewConsole.Debug("GetDCPlace Start", "DefaultDCProfileDAO.GetDCPlace");
                OneViewConsole.Debug("ServiceId :", JSON.stringify(ServiceId));
                OneViewConsole.Debug("UserId :", JSON.stringify(UserId));
                OneViewConsole.Debug("TemplateNodeId :", JSON.stringify(TemplateNodeId));
                OneViewConsole.Debug("DCDate :", JSON.stringify(DCDate));
                OneViewConsole.Debug("FromTime :", JSON.stringify(FromTime));
                OneViewConsole.Debug("ToTime :", JSON.stringify(ToTime));

                var DcPlaceList = [];
                var query = GetDCPlace_FaciltyQuery(ServiceId, UserId, TemplateNodeId, DCDate, FromTime, ToTime, DcPlaceDimension);
               ...
               // alert('DcProfileResult  : ' + JSON.stringify(DcProfileResult));
                ...
                for (var i = 0; i < DcProfileResult.length; i++) {
                    ...
                    if (resultDcPlace[0].ChildDbElementType == RCOType) {
                        ...
                        var resObj = { Id: DcPlaceNodeId, Name: DcPlaceName, DcPlaceDimension: DcProfileResult[i]['DcPlaceDimension'] };
        ...
        this.GetDCPlaceDimensionOLD = function (ServiceId, UserId, TemplateNodeId, DCDate, FromTime, ToTime) {
            try {
                OneViewConsole.Debug("GetDCPlace Start", "DefaultDCProfileDAO.GetDCPlace");
                OneViewConsole.Debug("ServiceId :", JSON.stringify(ServiceId));
                OneViewConsole.Debug("UserId :", JSON.stringify(UserId));
                ...
                var DcPlaceList = [];
                var query = GetDCPlace_FaciltyQuery(ServiceId, UserId, TemplateNodeId, DCDate, FromTime, ToTime,-1);
                var result = window.OneViewSqlite.excecuteSqlReader(query);
                var DcProfileResult = JSON.parse(result);

                //("DcProfileResult : " + JSON.stringify(DcProfileResult));
                var Incondition = "(";

                for (var i = 0; i < DcProfileResult.length; i++) {
                    Incondition += DcProfileResult[i].DcPlaceId;
                    Incondition += (i <= DcProfileResult.length - 2) ? "," : ")";
                }

                var DcPlaceDimensionQuery = "SELECT DISTINCT ChildDbElementType FROM OrganizationAssetsNode WHERE ServerId in" + Incondition;
                var DCPlaceDimensionList = window.OneViewSqlite.excecuteSqlReader(DcPlaceDimensionQuery);
              ...
              //  var IsCallQuery = false;
                ...
                for (var i = 0; i < DcPlaceIdDetailsList.length; i++) {
                    //if (DcPlaceIdDetailsList[i].DcPlaceId != 0) {
                     ...
                     //   IsCallQuery = true;
                        Incondition += DcPlaceIdDetailsList[i].DcPlaceId;
        ...
        this.GetAuditTemplateOLD = function (ServiceId, UserId, TemplateNewNodeId, DCDate, FromTime, ToTime, DcPlaceId) {
            try {
                ...
                var TemplateLst = [];
                var query = GetDCPlace_TemplateNodeQuery(ServiceId, UserId, TemplateNewNodeId, DCDate, FromTime, ToTime, DcPlaceId);
                var result = window.OneViewSqlite.excecuteSqlReader(query);
                var AuditTemplateResult = JSON.parse(result);

...
function LoginFacade(xlatService, $timeout) {
						...
						if (NetworkStatus.IsNetworkAvailable == true) {
							...
							if (_oUserDTO != null && _oUserDTO.IsAnyException == false) {
								...
								if (_oUserDTO.UserName != null) {
									...
									RegisterPushNotificationToServer(toaster);
	...
	var RegisterPushNotificationToServer = function (toaster) {
		try {
			if (PushNotificationRegistrationId != "" && PushNotificationRegistrationId != null && PushNotificationRegistrationId != undefined) {
				...
				//if (OneViewLocalStorage.Get(PushNotificationRegistrationId) == null) {
					...
					var _oAuthenticationServiceIL = new AuthenticationServiceIL(toaster);
					var RegisterDeviceInfoRslt = _oAuthenticationServiceIL.SendPushNotificationRegistrationToServer(RequestParam);
					if (RegisterDeviceInfoRslt != null) {
						if (RegisterDeviceInfoRslt.toUpperCase().trim() == "SUCCESS") {
...
function AuthenticationServiceIL(toaster) {
	...
	this.SendPushNotificationRegistrationToServer = function (RequestParam) {
		try {
			OneViewConsole.Debug("SendPushNotificationRegistrationToServer start", "AuthenticationServiceIL.SendPushNotificationRegistrationToServer");
			
			OneViewConsole.DataLog("Request from device : " + JSON.stringify(RequestParam), "ProfileDownloadIL.GetProfile");
			
			var _oOneViewChannel = new OneViewChannel();
			// _oOneViewChannel.toaster = toaster;
			_oOneViewChannel.url = oneViewGlobalVariables.FoodSafetyServiceURL + "UserProfileFacedService.svc/RegisterDeviceInfo_json";
			_oOneViewChannel.parameter = JSON.stringify({ "req": JSON.stringify(RequestParam) });
			var RegisterDeviceInfoRslt = _oOneViewChannel.Send();
			
			OneViewConsole.Debug("SendPushNotificationRegistrationToServer end", "AuthenticationServiceIL.SendPushNotificationRegistrationToServer");
			
			if (RegisterDeviceInfoRslt != null) {
				OneViewConsole.DataLog("Response from server : " + JSON.stringify(RegisterDeviceInfoRslt.RegisterDeviceInfo_jsonResult), "AuthenticationServiceIL.SendPushNotificationRegistrationToServer");
				return RegisterDeviceInfoRslt.RegisterDeviceInfo_jsonResult;
			}
			else {
				return RegisterDeviceInfoRslt
			}
		}
		catch (Excep) {
			throw oOneViewExceptionHandler.Create("IL", "AuthenticationServiceIL.AuthenticationServiceIL", Excep);
		}
	}
}

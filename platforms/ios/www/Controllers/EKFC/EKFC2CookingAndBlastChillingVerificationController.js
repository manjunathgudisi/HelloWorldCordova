...
MyApp.controller('CookingAndBlastChillingVerificationControllerEKFC2',
    function ($scope, $document, xlatService, $timeout, $location, $templateCache, snapRemote, $compile) {
        //oSetDefaultSpinner.Start();
        scope = $scope;
        oSnapRemote = snapRemote;
        var NCForm_Page = 'T66';
        // xlatService.setCurrentPage('CookingAndBlastChillingVerificationEKFC2');
        ...
        var oCookingAndBlastChillingVerificationFacade = new CookingAndBlastChillingVerificationFacadeEKFC2({ 'scope': $scope, 'document': $document, 'location': $location, 'xlatService': xlatService, 'toaster': '', 'SpinService': '', '$timeout': $timeout, 'compile': $compile });
...
function CookingAndBlastChillingVerificationFacadeEKFC2(parm) {
    try {
    ...
    var Loadddl = function () {
        try {
            ...
            var oProductddl = new AutoCompleteUserControl({ 'Scope': $scope, 'ControlId': 'AddlProductControlId', 'DataSourceModelName': 'Products', 'DisplayElementModelName': 'NewDCModel.AddlProductControlId', 'DATEntityTypeId': DATEntityType.RCO_WorkOrder, 'xlatService': xlatService, 'ToasterService': '', 'AttributeNodeId': 343 });
        ...
        try {
            ...
            for (var i = 0; i < _DcResultDetailsEntityLst.length; i++) {
                if (_DcResultDetailsEntityLst[i].IsDynamicAnswer == 'true') {
                    ...
                    //if (_DcResultDetailsEntityLst[i].AttributeNodeId == 343)//for product
                    //{

...
#define MyMainThreadAssert() NSAssert([NSThread isMainThread], @"MyProgressHUD needs to be accessed on the main thread.");

CGFloat const MyProgressMaxOffset = 1000000.f;

static const CGFloat MyDefaultPadding = 4.f;
static const CGFloat MyDefaultLabelFontSize = 16.f;
static const CGFloat MyDefaultDetailsLabelFontSize = 12.f;
static MyProgressHUD* hud = nil;

@protocol JS_MyProgressHUDController <JSExport>
-(void)ShowProgress;
-(void)HideProgress;
@end

@interface MyProgressHUD()<JS_MyProgressHUDController>
@end
...
@interface MyProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    MyProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    MyProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (MyProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            MyProgressHUD *hud = (MyProgressHUD *)subview;
...
- (void)ShowProgress {
	dispatch_async(dispatch_get_main_queue(), ^{
		...
		hud = [MyProgressHUD showHUDAddedTo:rootVC.view animated:YES];
	});
}
...
- (void)HideProgress {
	dispatch_async(dispatch_get_main_queue(), ^{
		CDVAppDelegate *appDelegate = [[UIApplication sharedApplication] delegate];
		UIViewController *rootVC = [[appDelegate window] rootViewController];
		[MyProgressHUD hideHUDForView:rootVC.view animated:YES];
	});
//	if (hud != nil) {
...
- (void)commonInit {
    ...
    _animationType = MyProgressHUDAnimationFade;
    _mode = MyProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(MyProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == MyProgressHUDAnimationZoom) {
        type = animatingIn ? MyProgressHUDAnimationZoomIn : MyProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == MyProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == MyProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == MyProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == MyProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    MyProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<MyProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    MyBackgroundView *backgroundView = [[MyBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = MyProgressHUDBackgroundStyleSolidColor;
    ...
    MyBackgroundView *bezelView = [MyBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:MyDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:MyDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [MyProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:MyDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[MyRoundProgressView class]];

    MyProgressHUDMode mode = self.mode;
    if (mode == MyProgressHUDModeIndeterminate) {
    ...
    else if (mode == MyProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[MyBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MyProgressHUDModeDeterminate || mode == MyProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[MyRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == MyProgressHUDModeAnnularDeterminate) {
            [(MyRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == MyProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MyProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[MyProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[MyProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[MyRoundProgressView class]]) {
        MyRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MyRoundProgressView appearanceWhenContainedIn:[MyProgressHUD class], nil];
#else
        appearance = [MyRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[MyProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((MyRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((MyRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[MyBarProgressView class]]) {
        MyBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MyBarProgressView appearanceWhenContainedIn:[MyProgressHUD class], nil];
#else
        appearance = [MyBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[MyProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((MyBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((MyBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
    MyBackgroundView *bezelView = self.bezelView;
...
- (void)layoutSubviews {
    ...
    // would trigger a zoMyie object access.
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? MyDefaultPadding : 0.f;
...
- (void)setMode:(MyProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == MyProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (void)updateForCurrentOrientationAnimated:(BOOL)animated {
    ...
    BOOL iOS8OrLater = kCFCoreFoundationVersionNuMyer >= kCFCoreFoundationVersionNuMyer_iOS_8_0;
...
@implementation MyRoundProgressView
...
@implementation MyBarProgressView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = MyProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:(MyProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    MyProgressHUDBackgroundStyle style = self.style;
    if (style == MyProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == MyProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation MyProgressHUDRoundedButton

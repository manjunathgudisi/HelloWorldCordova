...
MyApp.controller('OneviewIncidentReportController',
    ...
    var oIncidentReportFacade = new OneviewIncidentReportFacade({ 'scope': $scope, 'document': $document, 'location': $location, 'xlatService': xlatService, 'toaster': '', 'SpinService': '', '$timeout': $timeout, 'compile': $compile });
    ...
    $scope.OnPredefinedActionChangedEvent = function (PredefinedAction) {
        
        //alert('OnPredefinedActionChangedEvent');
        //alert('PredefinedAction : ' + JSON.stringify(PredefinedAction));

        //oIncidentReportFacade.ChangePredefinedActionList(PredefinedAction);
    }
...
function OneviewIncidentReportFacade(parm) {
    ...
    try {
        ...
        var TemplateId = "146";
        var TemplateName = "Incident Report";
        ...
        OneViewSessionStorage.Save("TemplateId", "146");
        OneViewSessionStorage.Save("TemplateName", "Incident Report");
    ...
    this.Init = function () {
        try {
            ...
            $scope.PredefinedActionList = [];

            $scope.ActionNCConfigSetting = null
    ...
    var LoadPredefinedActions = function () {
        try {
            var PredefinedActionIDs = [];
            $scope.PredefinedActionList = [];

            if ($scope.ActionNCConfigSetting.ActionList != null) {
                for (var i = 0; i < $scope.ActionNCConfigSetting.ActionList.length ; i++) {
                    var ActionDetails = $scope.ActionNCConfigSetting.ActionList[i];
                    if (ActionDetails.ActionConfig.PreDefinedActionMasterId != undefined) {
                        PredefinedActionIDs.push(ActionDetails.ActionConfig.PreDefinedActionMasterId);
                    }
                }
            }

            var _ActionDAO = new ActionDAO();
            var ResponseList = _ActionDAO.GetAllByIds(PredefinedActionIDs);
            //alert('ResponseList : ' + JSON.stringify(ResponseList));

            if (ResponseList != null && ResponseList.length > 0) {
                $scope.PredefinedActionShow = true;
                for (var i = 0; i < ResponseList.length; i++) {
                    $scope.PredefinedActionList.push({ 'label': ResponseList[i].Name, id: 0, "PreDefinedActionId": ResponseList[i].ServerId, selected: "" })
                    
                }
            }
            else {
                $scope.PredefinedActionShow = false;
            }
            ...
            if (DcId != null) {
                ...
                //alert('LoadPredefinedActions $scope.ActionNCConfigSetting : ' + JSON.stringify($scope.ActionNCConfigSetting));              
                
            }

            //alert('$scope.PredefinedActionList : ' + JSON.stringify($scope.PredefinedActionList));
        }
        catch (Excep) {
            //alert('LoadPredefinedActions Excep :' + Excep)
            throw Excep;
        }
    }

    this.ChangePredefinedActionList = function (PredefinedAction) {
        try {
            OneViewConsole.Debug("ChangePredefinedActionList Start", "IncidentReportFacade.ChangePredefinedActionList");

           

            
            OneViewConsole.Debug("ChangePredefinedActionList End", "IncidentReportFacade.ChangePredefinedActionList");
        }

        catch (Excep) {
            oOneViewExceptionHandler.Catch(Excep, "IncidentReportFacade.ChangePredefinedActionList", xlatService);
        }
    }
    ...
    this.SaveDCRecords = function (IsSubmit) {
        ...
        try {
            ...
            //}           
            //else {
            //    alert("MN-RQ-OIR-001 :: Please enter incident and incident type");
            //}
        ...
        try {
            ...
            if (DataCaptureClientGuid != "") {
                ...
                if (DCNCMappingResult.length > 0) {
                    ...
                    //alert('ActionDetailsResult : ' + JSON.stringify(ActionDetailsResult));
                    var SelectedPredefinedActionIds = [];
                    for (var i = 0; i < ActionDetailsResult.length; i++) {
                        if (ActionDetailsResult[i].PreDefinedActionId == 0) {
                            $scope.CustomCorrectiveActionList.push({ label: ActionDetailsResult[i].CustomAction, id: ActionDetailsResult[i].Id });
                        }
                        else {
                            SelectedPredefinedActionIds.push({ "PreDefinedActionId": ActionDetailsResult[i].PreDefinedActionId, id: ActionDetailsResult[i].Id });
                            $scope.PreCustomCorrectiveActionList.push({ label: '', id: ActionDetailsResult[i].Id });
                        }
                    }

                    GetPredefinedActionDetails(SelectedPredefinedActionIds);

                    //alert('LoadCorrectiveActions $scope.PredefinedActionList : ' + JSON.stringify($scope.PredefinedActionList));
    ...
    var GetPredefinedActionDetails = function (SelectedPredefinedActionIds) {
        try {
            var List = [];
            for (var i = 0; i < $scope.PredefinedActionList.length; i++) {
                List.push($scope.PredefinedActionList[i].PreDefinedActionId);
            }

            $scope.PredefinedActionList = [];

            var _ActionDAO = new ActionDAO();
            var ResponseList = _ActionDAO.GetAllByIds(List);
            //alert('GetPredefinedActionDetails ResponseList : ' + JSON.stringify(ResponseList));
            //alert('SelectedPredefinedActionIds : ' + JSON.stringify(SelectedPredefinedActionIds));

            if (ResponseList != null && ResponseList.length > 0) {              
                for (var i = 0; i < ResponseList.length; i++) {
                    var sel = "";
                    var id = 0;
                    for (var j = 0; j < SelectedPredefinedActionIds.length ; j++) {
                        if (ResponseList[i].ServerId == SelectedPredefinedActionIds[j].PreDefinedActionId) {
                            sel = true;
                            id = SelectedPredefinedActionIds[j].id;
                            break;
                        }
                    }
                    $scope.PredefinedActionList.push({ 'label': ResponseList[i].Name, id: id, "PreDefinedActionId": ResponseList[i].ServerId, selected: sel })
                }
            }
        }
    ...
    var CorrectiveActionEventHandler = function () {
        ...
        try {
            ...
            //alert($scope.CustomCorrectiveActionList.length + "," + $scope.CustomObservationList.length);
            //alert($scope.PredefinedActionList.length + ' , $scope.PredefinedActionList final : ' + JSON.stringify($scope.PredefinedActionList));

            if ($scope.PredefinedActionList.length > 0) {
                for (var i = 0; i < $scope.PredefinedActionList.length ; i++) {
                    if ($scope.PredefinedActionList[i].selected == true || $scope.PredefinedActionList[i].selected == "selected") {
                       
                        $scope.CustomCorrectiveActionList.push({ id: $scope.PredefinedActionList[i].id, "PreDefinedActionId": $scope.PredefinedActionList[i].PreDefinedActionId, "label": "" });
                    }
                }
            }

            //alert($scope.CustomCorrectiveActionList.length + ' , $scope.CustomCorrectiveActionList final : ' + JSON.stringify($scope.CustomCorrectiveActionList));
    ...
    var UpdateObservation = function (IsCorrectiveAction) {
        try {
            ...
            _oDataCaptureBO.DCNCMappingList = [];
            ...
            if (DataCaptureClientGuid != ""){
                ...
                if (DCNCMappingResult.length > 0) {
                    if (IsCorrectiveAction == true) {
                        ...
                        //Update DCNCMapping
                        var QueryDCNCMapping = "UPDATE DCNCMapping SET NCRuleId=" + $scope.ActionNCConfigSetting.RuleId + ", RuleName='" + $scope.ActionNCConfigSetting.RuleName +
                   + "', RuleDescription='" + $scope.ActionNCConfigSetting.RuleDescription         
                    + "' , TemplateNodeIds ='" + $scope.ActionNCConfigSetting.TemplateNodeIds + "'"
                    + " WHERE DataCaptureClientGuid='" + DataCaptureClientGuid + "'";


                        //alert(QueryDCNCMapping);
                        window.OneViewSqlite.excecuteSql(QueryDCNCMapping);
                        ...
                        for (var j = 0; j < $scope.CustomCorrectiveActionList.length; j++) {
                            if ($scope.CustomCorrectiveActionList[j].id == 0) {
                                ...
                                if ($scope.CustomCorrectiveActionList[j].PreDefinedActionId == undefined) {
                                    ...
                                    _oActionDetailsEntity.PreDefinedActionId = $scope.CustomCorrectiveActionList[j].PreDefinedActionId;
                                }
    ...
    var SetActionDTO = function (ActionDetailsLst) {
        ...
        try {
            ...
            //alert(_oDataCaptureBO.DCNCMappingList.length + '_oDataCaptureBO.DCNCMappingList : ' + JSON.stringify(_oDataCaptureBO.DCNCMappingList));       
            //alert('$scope.ActionNCConfigSetting : ' + JSON.stringify($scope.ActionNCConfigSetting));
            ...
            _oDCNCMapping.NCRuleId = $scope.ActionNCConfigSetting.RuleId;
            _oDCNCMapping.RuleGroup = $scope.ActionNCConfigSetting.RuleGroup;
            

            _oDCNCMapping.RuleName = $scope.ActionNCConfigSetting.RuleName,
            _oDCNCMapping.RuleDescription = $scope.ActionNCConfigSetting.RuleDescription,
            _oDCNCMapping.AttributeGroupId = 0;
            _oDCNCMapping.RuleCode = $scope.ActionNCConfigSetting.RuleCode,
            _oDCNCMapping.Deviatedby = '';
            _oDCNCMapping.ExpectedValue = '';
            _oDCNCMapping.ActualValue = '';
            _oDCNCMapping.TemplateNodeIds = $scope.ActionNCConfigSetting.TemplateNodeIds
            ...
            for (var i = 0; i < ActionDetailsLst.length; i++) {
                ...
                //alert(ActionDetailsLst[i].PreDefinedActionId);
                if (ActionDetailsLst[i].PreDefinedActionId != undefined) {
                    _oActionDetailsEntity.PreDefinedActionId = ActionDetailsLst[i].PreDefinedActionId;
                }
                else {
            ...
            //alert('SetActionDTO Excep : '+ Excep +  JSON.stringify(Excep));
            throw Excep;
        }
        finally {
            oProductddl = null;
    ...
    var Loadddl = function () {
        try {
            ...
            var oObservationTypeddl = new AutoCompleteUserControl({ 'Scope': $scope, 'ControlId': 'AddlIncidentTypeControlId', 'DataSourceModelName': 'IncidentType', 'DisplayElementModelName': 'NewDCModel.AddlIncidentTypeControlId', 'DATEntityTypeId': DATEntityType.IncidentType, 'xlatService': xlatService, 'ToasterService': '', 'IsDynamicElementCreationEnabled': false, 'MinCharToSearch': 0, 'SelectedIndexChangedEventHandler': oObservationTypeSelectedIndexChanged, 'AttributeNodeId': 148 });
            oObservationTypeddl.SetDataSourceWithObservationType(OrganizationId, DATEntityType.IncidentType, _TableNamesEnum.OrganizationAssetsNode);
    ...
    var GetNCForIncidentReport = function () {
        try {
            OneViewConsole.Debug("GetNCForIncidentReport Start", "IncidentReportFacade.GetNCForIncidentReport");

            var _oCPActionNCComponent = new CPActionNCComponent($scope, TemplateNodes, '', xlatService);
            var ActionNCConfigSettingList = _oCPActionNCComponent.GetActionNCStatus(148, false);

            //alert('ActionNCConfigSettingList : ' + JSON.stringify(ActionNCConfigSettingList));

            for (var i = 0 ; i < ActionNCConfigSettingList.length ; i++) {
                if (ActionNCConfigSettingList[i].ActionNCConfigSetting.IsManual == false) {
                    if (ActionNCConfigSettingList[i].ActionNCConfigSetting.IsNC == true && ActionNCConfigSettingList[i].IsRuleViolated == true) {
                        $scope.ActionNCConfigSetting = ActionNCConfigSettingList[i].ActionNCConfigSetting;
                        break;
                    }
                }
            }

            OneViewConsole.Debug("GetNCForIncidentReport End", "IncidentReportFacade.GetNCForIncidentReport");
        }
        catch (Excep) {
            throw Excep;
        }
    ...
    var LoadAnswerModes = function () {
        try {
            ...
            oRefrigeratorStatus.AnswerModes(TemplateNodes, 150);

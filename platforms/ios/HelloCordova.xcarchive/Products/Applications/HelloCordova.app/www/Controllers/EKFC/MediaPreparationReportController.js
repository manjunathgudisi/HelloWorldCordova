...
MyApp.controller('MediaPreparationReportController',
    function ($scope, $document, xlatService, $timeout, $location, $templateCache, snapRemote, $compile) {
    //oSetDefaultSpinner.Start();
        // xlatService.setCurrentPage('MediaPreparationReport');
	...
	var oStaffFoodTempFacade = new MediaPreparationReportFacade({ 'scope': $scope, 'document': $document, 'Mylocation': $location, 'xlatService': xlatService, 'toaster': '', 'SpinService': '', 'compile': $compile, '$timeout': $timeout });
...
function MediaPreparationReportFacade(parm) {
      ...
      //  var _oNCComponent = new NCComponent($scope, TemplateNodes, '', xlatService);
        ...
        var LoadAnswerModes = function () {
            try {
                ...
                var oTypeofMedia = new AnswerModeUserControl({ 'Scope': $scope, 'ControlId': 'chkTypeofMedia', 'DataSourceModelName': 'TypeofMedia', 'DisplayElementModelName': 'NewDCModel.chkTypeofMedia' });
                oTypeofMedia.AnswerModes(TemplateNodes, 622);

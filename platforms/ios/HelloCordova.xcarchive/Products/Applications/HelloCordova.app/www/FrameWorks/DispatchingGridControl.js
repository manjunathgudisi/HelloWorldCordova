...
function DispatchingGridControl() {
            ...
            try {
                ...
                if (DataSource.length != 0) {
                    ...
                    for (var i = 0; i < NormalizedData.length ; i++) {
                        ...
                        var Row_Start = '<div id="Row_' + i + '" DataCaptureId="' + NormalizedData[i].DataCaptureId + '" class="' + RowCss + '" onclick="new DispatchingViewRecordsFacade().RowClick(' + NormalizedData[i].DataCaptureId + ',' + i + ',' + IsCompleted + ',' + IsSynchronized + ');">';
                        var Row_Content = GetDataRow(NormalizedData[i], HeaderMetaData);
                        var Row_End = "</div>";
        ...
        var GetDataColumn = function (html, HeaderMetaData, RowData) {
            try {
                ...
                if (HeaderMetaData.SubDataFields != undefined) {
                    ...
                    else if (HeaderMetaData.IsDefaultValueButton == true) {
                        html = html + '<div onclick="DefaultValueClick(' + RowData.DataCaptureId + ')" id="' + HeaderMetaData.FieldName + RowData.DataCaptureId + '" class="' + GetStyle(HeaderMetaData) + '" name="' + HeaderMetaData.FieldName + '"><a snap-toggle="right" class="button button-block row-edit"><i class="icon icon-arrow-loop"></i></a></div>';
                    }
                    else if (HeaderMetaData.IsEditButton == true) {
                        html = html + '<div onclick="EditButtonClick(' + RowData.DataCaptureId + ')" id="' + HeaderMetaData.FieldName + RowData.DataCaptureId + '" class="' + GetStyle(HeaderMetaData) + '" name="' + HeaderMetaData.FieldName + '"><a snap-toggle="right" class="button button-block row-edit"><i class="icon icon-edit"></i></a></div>';
                    }
                    else {
                        var FieldName = "'" + HeaderMetaData.FieldName + "'";
                        html = html + '<div id="' + HeaderMetaData.FieldName + RowData.DataCaptureId + '" class="' + GetStyle(HeaderMetaData) + '" name="' + HeaderMetaData.FieldName + '" onclick="new DispatchingViewRecordsFacade().SetAutoTemperatureListener(' + FieldName + ',' + RowData.DataCaptureId + ')"><span id="Span_' + HeaderMetaData.FieldName + RowData.DataCaptureId + '">' + ColumnValue + '</span></div>';
                        //html = html + '<div id="' + HeaderMetaData.FieldName + RowData.DataCaptureId + '" class="' + GetStyle(HeaderMetaData) + '" name="' + HeaderMetaData.FieldName + '"><span id="Span_' + HeaderMetaData.FieldName + RowData.DataCaptureId + '">' + ColumnValue + '</span></div>';
                    }
                }

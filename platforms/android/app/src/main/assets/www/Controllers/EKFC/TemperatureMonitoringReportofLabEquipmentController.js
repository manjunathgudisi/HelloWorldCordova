...
MyApp.controller('TemperatureMonitoringReportofLabEquipmentController',
    function ($scope, $document, xlatService, $timeout, $location, $templateCache, snapRemote, $compile) {
    //oSetDefaultSpinner.Start();
        // xlatService.setCurrentPage('TemperatureMonitoringReportofLabEquipment');
	...
	var oStaffFoodTempFacade = new TemperatureMonitoringReportofLabEquipmentFacade({ 'scope': $scope, 'document': $document, 'Mylocation': $location, 'xlatService': xlatService, 'toaster': '', 'SpinService': '', 'compile': $compile, '$timeout': $timeout });
    ...
    $scope.CalculateUnitReference = function (Unitofreference, MeasuredTemp, SetTemperature,MinObservedTemperature,MaxObservedTemperature) {

        oStaffFoodTempFacade.CalculateUnitReference(Unitofreference, MeasuredTemp, SetTemperature, MinObservedTemperature, MaxObservedTemperature);
    }
...
function TemperatureMonitoringReportofLabEquipmentFacade(parm) {
      ...
      //  var _oNCComponent = new NCComponent($scope, TemplateNodes, '', xlatService);
        ...
        var Loadddl = function () {
            try {
                ...
                //var oEquipmentNameddl = new AutoCompleteUserControl({ 'Scope': $scope, 'ControlId': 'AddlEquipmentNameControlId', 'DataSourceModelName': 'EquipmentName', 'DisplayElementModelName': 'NewDCModel.AddlEquipmentNameControlId', 'DATEntityTypeId': DATEntityType.RCOMaster_EquipmentName, 'IsDynamicElementCreationEnabled': true, 'xlatService': xlatService, 'ToasterService': '', 'MinCharToSearch': 0, 'AttributeNodeId': 713, 'SelectedIndexChangedEventHandler': oEquipmentNameSelectedIndexChanged });
                var oEquipmentNameddl = new AutoCompleteUserControl({ 'Scope': $scope, 'ControlId': 'AddlEquipmentNameControlId', 'DataSourceModelName': 'EquipmentName', 'DisplayElementModelName': 'NewDCModel.AddlEquipmentNameControlId', 'DATEntityTypeId': DATEntityType.RCOMaster_LabEquipment, 'IsDynamicElementCreationEnabled': false, 'xlatService': xlatService, 'ToasterService': '', 'MinCharToSearch': 0, 'AttributeNodeId': 713, 'SelectedIndexChangedEventHandler': oEquipmentNameSelectedIndexChanged });
                ...
                oEquipmentNameddl.DefaultRefreshJobs.push({ Type: 'DefaultTextboxRefreshJob', ControlId: 'txtGlassthermometerControlId', ColumnNames: ['Column1'], Seperater: '-' });
                oEquipmentNameddl.DefaultRefreshJobs.push({ Type: 'DefaultTextboxRefreshJob', ControlId: 'ATSetTemperatureControlId', ColumnNames: ['Column2'], Seperater: '-' });

                oEquipmentNameddl.SetDataSourceFromTreeChildsWithType(OrganizationId, DATEntityType.RCOMaster_LabEquipment, _TableNamesEnum.OrganizationAssetsNode);
               

                //var oEquipmentNumberddl = new AutoCompleteUserControl({ 'Scope': $scope, 'ControlId': 'AddlEquipmentNumberControlId', 'DataSourceModelName': 'EquipmentNumber', 'DisplayElementModelName': 'NewDCModel.AddlEquipmentNumberControlId', 'DATEntityTypeId': DATEntityType.RCOMaster_EquipmentNumber, 'IsDynamicElementCreationEnabled': true, 'xlatService': xlatService, 'ToasterService': '', 'MinCharToSearch': 0, 'AttributeNodeId': 714 });
        ...
        var oEquipmentNameSelectedIndexChanged = function (EventArgs) {
            try {
                OneViewConsole.Debug("oEquipmentNameSelectedIndexChanged Start", "oStaffFoodTempFacade.oEquipmentNameSelectedIndexChanged");
                ...
                if (EventArgs != null && EventArgs.NewValue.Id != null && EventArgs.NewValue.Id > 0) {
                    ...
                    //}
                    if ($scope.AddlEquipmentNameControlId.GetSelectedText() != '') {
                        var EquipmentName = $scope.AddlEquipmentNameControlId.GetSelectedText();
                        EquipmentName = EquipmentName.toUpperCase();
                        if (EquipmentName.indexOf("REFRIGERATOR") != -1) {                        
                            $scope.SetTemperatureShow = false;
                            $scope.MinObservedTemperatureShow = true;
                            $scope.MaxObservedTemperatureShow = true;
                            $scope.NewDCModel.ATSetTemperatureControlId = "";
                            if ($scope.NewDCModel.ATObservedTempControlId == "") {
                                $scope.NewDCModel.ATObservedTempControlId = 0;
                            }
                            if ($scope.NewDCModel.ATMaxObservedTempControlId == "") {
                                $scope.NewDCModel.ATMaxObservedTempControlId = 5;
                            }
                            MyInstance.CalculateUnitReference('txtUnitofreferenceControlId', 'ATMeasuredTempControlId', '', 'ATObservedTempControlId', 'ATMaxObservedTempControlId');
                        }
                        else {
                            $scope.MinObservedTemperatureShow = false;
                            $scope.MaxObservedTemperatureShow = false;
                            $scope.SetTemperatureShow = true;
                            $scope.NewDCModel.ATObservedTempControlId = "";
                           $scope.NewDCModel.ATMaxObservedTempControlId = "";
                            MyInstance.CalculateUnitReference('txtUnitofreferenceControlId', 'ATMeasuredTempControlId', 'ATSetTemperatureControlId', '', '');
                        }
                    }
                }
                else {
                ...
                OneViewConsole.Debug("oEquipmentNameSelectedIndexChanged End", "oStaffFoodTempFacade.oEquipmentNameSelectedIndexChanged");
            }
            catch (Excep) {
                throw Excep;
            }
        ...
        this.CalculateUnitReference = function (Unitofreference, MeasuredTemp, SetTemperature, MinObservedTemperature, MaxObservedTemperature) {
            try {
                //$scope.NewDCModel["ObservationComments"] = '';
                Unitofreference = $scope.NewDCModel["txtUnitofreferenceControlId"];
                MeasuredTemp = $scope.NewDCModel["ATMeasuredTempControlId"];
                if (($scope.AddlEquipmentNameControlId.GetSelectedText() != "" && $scope.AddlEquipmentNameControlId.GetSelectedText() != undefined)) {
                    var EquipmentName = $scope.AddlEquipmentNameControlId.GetSelectedText();
                    EquipmentName = EquipmentName.toUpperCase();
                    if (EquipmentName.indexOf("REFRIGERATOR") != -1) {
                        //if ($scope.AddlEquipmentNameControlId.GetSelectedText() == "Refrigerator") {
                        if (($scope.NewDCModel["ATObservedTempControlId"] !== "" && $scope.NewDCModel["ATObservedTempControlId"] !== undefined) && ($scope.NewDCModel["ATMaxObservedTempControlId"] != "" && $scope.NewDCModel["ATMaxObservedTempControlId"] != undefined) && (MeasuredTemp != "" && MeasuredTemp != undefined)) {

                            MeasuredTemp = parseFloat($scope.NewDCModel["ATMeasuredTempControlId"]);
                            MinObservedTemperature = parseFloat($scope.NewDCModel["ATObservedTempControlId"]);
                            MaxObservedTemperature = parseFloat($scope.NewDCModel["ATMaxObservedTempControlId"]);
                            if ((MeasuredTemp >= (MinObservedTemperature - 2) && MeasuredTemp <= (MaxObservedTemperature + 2))) {
                                $scope.NewDCModel["txtUnitofreferenceControlId"] = "WT";
                            }
                            else {
                                $scope.NewDCModel["txtUnitofreferenceControlId"] = "OOT";
                            }
                        }
                        else {
                            $scope.NewDCModel["txtUnitofreferenceControlId"] = "";
                        }
                    }
                    else {
                        if (($scope.NewDCModel["ATSetTemperatureControlId"] != "" && $scope.NewDCModel["ATSetTemperatureControlId"] != undefined) && (MeasuredTemp != "" && MeasuredTemp != undefined)) {

                            SetTemperature = parseFloat($scope.NewDCModel["ATSetTemperatureControlId"]);
                            MeasuredTemp = parseFloat($scope.NewDCModel["ATMeasuredTempControlId"]);

                            if ((MeasuredTemp >= (SetTemperature - 1) && MeasuredTemp <= (SetTemperature + 1))) {
                                $scope.NewDCModel["txtUnitofreferenceControlId"] = "WT";
                            }
                            else {
                                $scope.NewDCModel["txtUnitofreferenceControlId"] = "OOT";
                ...
                oOneViewExceptionHandler.Catch(Excep, "TemperatureMonitoringReportofLabEquipmentFacade.CalculateUnitReference", xlatService);
            }
        }
}

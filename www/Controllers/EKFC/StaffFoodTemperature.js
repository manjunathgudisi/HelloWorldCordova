...
MyApp.controller('StaffFoodTemperature',
	...
	var oStaffFoodTempFacade = new StaffFoodTempFacade({ 'scope': $scope, 'document': $document, 'Mylocation': $location, 'xlatService': xlatService, 'toaster': '', 'SpinService': '', 'compile': $compile, '$timeout': $timeout });
...
function StaffFoodTempFacade(parm) {
      ...
      //  var _oNCComponent = new NCComponent($scope, TemplateNodes, '', xlatService);
        ...
        var Loadddl = function () {
            try {
                ...
                var oProductddl = new AutoCompleteUserControl({ 'Scope': $scope, 'ControlId': 'AddlCanteen', 'DataSourceModelName': 'Products', 'DisplayElementModelName': 'NewDCModel.AddlCanteen', 'DATEntityTypeId': DATEntityType.RCOMaster_Canteen, 'IsDynamicElementCreationEnabled': false, 'xlatService': xlatService, 'ToasterService': '' ,'MinCharToSearch': 0, 'AttributeNodeId': 434 });
                oProductddl.SetDataSourceFromTreeChildsWithType(DcPlaceId, DATEntityType.RCOMaster_Canteen, _TableNamesEnum.OrganizationAssetsNode);

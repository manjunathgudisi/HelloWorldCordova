...
MyApp.controller('FoodSafetyShiftReportRev2Controller',
    function ($scope, $document, xlatService, $timeout, $location, $templateCache, snapRemote, $compile) {
       // debugger;
        ...
        var query = "SELECT * FROM DcProfileEntity WHERE DcUserId= " + UserId + " AND TemplateNodeId = 15629";
        ...
        if (IsNavigate == true) {
            ...
            var oFoodSafetyShiftReportRev2Facade = new FoodSafetyShiftReportRev2Facade({ 'scope': $scope, 'document': $document, 'location': $location, 'xlatService': xlatService, 'toaster': '', 'SpinService': '', '$timeout': $timeout, 'compile': $compile });

            oFoodSafetyShiftReportRev2Facade.Init();
            oFoodSafetyShiftReportRev2Facade.PageLoad();
    ...
    $scope.$on('$destroy', function () {
        ...
        if (IsNavigate == true) {
            oFoodSafetyShiftReportRev2Facade.Destroy();
        }
    ...
    $scope.GetProbeStatus = function () {
        oFoodSafetyShiftReportRev2Facade.GetProbeStatus();
    };

    $scope.AddRecords = function () {
        oFoodSafetyShiftReportRev2Facade.SaveDCRecords(false);
    };
    
    $scope.SetSelectedTextBoxColor = function (ControlId) {
        oFoodSafetyShiftReportRev2Facade.SetSelectedTextBoxColor(ControlId);
    };
    ...
    $scope.SetAutoTemperatureListener = function (EventArg, TimeModelId) {
        oFoodSafetyShiftReportRev2Facade.SetAutoTemperatureListener(EventArg, TimeModelId);
    };
    ...
    $scope.Temperature_NgKeyUp = function (AttributeId, ControlId, RefreshcontrolId, IsNc) {
        ...
        lastTimeOutId = $timeout(function () { oFoodSafetyShiftReportRev2Facade.Temperature_NgKeyUp(AttributeId, ControlId, RefreshcontrolId, IsNc); }, Timeout);
    }
    ...
    $scope.ClearForm = function () {
        oFoodSafetyShiftReportRev2Facade.ClearForm();
    }

    $scope.$on('$destroy', function () {
        scope = null;
       // ionicBackdrop = null;
        oFoodSafetyShiftReportRev2Facade = null;
        $scope = null;
    });

    $scope.PreControlEvents = function (AttributeId, ControlId, $event) {
        oFoodSafetyShiftReportRev2Facade.PreControlEvents(AttributeId, ControlId, $event);
    }

    $scope.PostControlEvents = function (AttributeId, ControlId) {

        oFoodSafetyShiftReportRev2Facade.PostControlEvents(AttributeId, ControlId);
    }

    $scope.ngChange_setDateTime = function (ControlId) {
        oFoodSafetyShiftReportRev2Facade.ngChange_setDateTime(ControlId);
    }

    $scope.CustomNCClick = function () {
        oFoodSafetyShiftReportRev2Facade.CustomNCClick();
    }

    $scope.NCClick = function () {
        oFoodSafetyShiftReportRev2Facade.NCClick($compile);
    }

    $scope.ObservationTabClick = function () {
        oFoodSafetyShiftReportRev2Facade.ObservationTabClick($compile);
    }

    $scope.NCTabClick = function () {
        oFoodSafetyShiftReportRev2Facade.NCTabClick($compile);
    }

    $scope.CloseRightPanel = function () {
        oFoodSafetyShiftReportRev2Facade.CloseRightPanel();
    }

    $scope.SubmitRecords = function () {
        oFoodSafetyShiftReportRev2Facade.SaveDCRecords(true);
    }

    $scope.ClearReasons = function () {
        oFoodSafetyShiftReportRev2Facade.ClearReasons();
    }

    $scope.ClearComments = function () {
        oFoodSafetyShiftReportRev2Facade.ClearComments();
    }

    $scope.ProbeTesting = function () {
        oFoodSafetyShiftReportRev2Facade.ProbeTesting();
    };
    ...
    $scope.AttachPicture = function () {
        ...
        _oOneViewCordovaCameraPlugin.CaptureImage(function (_ImageURL) {
            oFoodSafetyShiftReportRev2Facade.AttachPicture(_ImageURL);
        });
    }
    ...
    $scope.BrowsePicture = function () {
        ...
        _oOneViewCordovaCameraPlugin.BrowsePicture(function (_ImageURL) {
            ...
            oFoodSafetyShiftReportRev2Facade.AttachPicture(NewPath);
        });
    }
...
function FoodSafetyShiftReportRev2Facade(parm) {
    ...
    try {
        ...
        var TemplateId = "15629";
        var TemplateName = "Food Safety Spot Check Report";
        ...
        OneViewSessionStorage.Save("TemplateId", "15629");
        OneViewSessionStorage.Save("TemplateName", "Food Safety Spot Check Report");
        ...
        var IsFromSaveorEditClick = false;
    ...
    var GenerateAssignedTo_ToEmail_ToCC = function () {
        try {
          
            for (var i = 1; i < 21; i++) {
                if (AssignedTo_ToEmail_ToCC[i] == undefined) {
                    AssignedTo_ToEmail_ToCC[i] = {ToEmail:"",ToCC:"",RCoId:0,RCoName:""};
                }
            }
        ...
        try {
           ...
           // var ModelIdForAutoTimeUpdation;
            ...
            if (DcId != null) {
                ...
                ShowAssignedTo_ToEmail_ToCC(15669, "chkAssignedTo");
            }
            else {               
                $scope.Add = 'Add';
    ...
    var SetRcoDataSourceForBand = function (DcPlaceId) {

        try {
            OneViewConsole.Debug("SetRcoDataSourceForBand Start", "CustomerComplaintMonitoringFacade.SetRcoDataSourceForBand");

            var Result = [];
            var Query = "SELECT ServerId AS AssetServerId,ChildDbElementName AS AssetName FROM OrganizationAssetsNode WHERE ChildDbElementType = " + DATEntityType.RCOMaster_Department;
            if (DcPlaceId != undefined) {
                var _DefaultTreeDAO = new DefaultTreeDAO();
                var ParentNode = _DefaultTreeDAO.GetNodeById(DcPlaceId, _TableNamesEnum.OrganizationAssetsNode);
                ...
                if (ParentNode.length != 0) {
                    ...
                    Query = "SELECT AssetNode.ServerId AS AssetServerId,AssetNode.ChildDbElementName AS AssetName,RcoMaster.Column1,RcoMaster.Column2 FROM OrganizationAssetsNode AssetNode" +
                            " INNER JOIN RcoMasterEntity RcoMaster ON AssetNode.ChildDbElementId=RcoMaster.ServerId" +
                            " WHERE Left > " + ParentNode[0].Left + " AND Right < " + ParentNode[0].Right + " AND ChildDbElementType = '" + DATEntityType.RCOMaster_Department + "' ORDER BY AssetNode.ChildDbElementName ASC";
                }

                Band_ServiceID_1[100001] = {
                };
            ...
            if (Result.length > 0) {
                ...
                AssignedTo_ToEmail_ToCC = {};
                GenerateAssignedTo_ToEmail_ToCC();           


                for (var i = 0; i < Result.length; i++) {

                    Band_ServiceID_1[100001][Result[i].AssetServerId] = {
                        'Name': Result[i].AssetName,
                        'Value': Result[i].AssetServerId,
                        'Sequence': (i + 1),
                        'ColourIndex': 'green'
                    };


                    if (AssignedTo_ToEmail_ToCC[i + 1] != undefined) {
                        AssignedTo_ToEmail_ToCC[i + 1].RCoId = Result[i].AssetServerId;
                        AssignedTo_ToEmail_ToCC[i + 1].RCoName = Result[i].AssetName;
                        AssignedTo_ToEmail_ToCC[i + 1].ToEmail = Result[i].Column1;
                        AssignedTo_ToEmail_ToCC[i + 1].ToCC = Result[i].Column2;
                    }
                }
                

                var TemplateNodeObject = TemplateNodes[15669];
                if (TemplateNodeObject.AnswerMode[0].BandInfo != undefined) {
                    TemplateNodeObject.AnswerMode[0].BandInfo = {};
                    TemplateNodeObject.AnswerMode[0].BandInfo = Band_ServiceID_1[100001];
                }


                if ($scope["chkAssignedTo"]!=undefined && $scope["chkAssignedTo"].GetDataSource() != undefined) {
                    ClearMultiSelectAnswerModes();
                   
                 }
                HideAndClearAssignedTo_ToEmail_ToCC();
                LoadMultiSelectAnswerModes();
    ...
    var HideAndClearAssignedTo_ToEmail_ToCC = function () {
        try {
            OneViewConsole.Debug("ShowAssignedTo_ToEmail_ToCC Start", "IncidentReportFacade.ShowAssignedTo_ToEmail_ToCC");

       
           for (var i = 1; i < 21; i++) {
               $scope["Div_" + i] = false;

               $scope.NewDCModel["txtToEmailAddress_" + i] = "";
               $scope.NewDCModel["txtCCEmailAddress_" + i] = "";
           }
              
            OneViewConsole.Debug("ShowAssignedTo_ToEmail_ToCC End", "IncidentReportFacade.ShowAssignedTo_ToEmail_ToCC");
        }
        catch (Excep) {
            throw Excep;
        }
    ...
    var LoadMultiSelectAnswerModes = function () {
        try {
            OneViewConsole.Debug("LoadMultiSelectAnswerModes Start", "VictoryEquipmentSanitizerEKFC2ControllerFacadeCopy.LoadMultiSelectAnswerModes");

            var _oBandIntializeFramework = new BandIntializeFramework();
            _oBandIntializeFramework.MultiSelectIntialize(TemplateNodes, $scope, MyInstance.Temperature_NgKeyUp);

            OneViewConsole.Debug("LoadMultiSelectAnswerModes End", "VictoryEquipmentSanitizerEKFC2ControllerFacadeCopy.LoadMultiSelectAnswerModes");
        }
        catch (Excep) {
            throw Excep;
        }
        ...
        try {
            ...
            if (ObservationList.length > 0 || $scope.CustomObservationList.length > 0) {
                ...
                if (DcId != null) {
                    ...
                    if (Status != false) {
                        IsFromSaveorEditClick = false;
                        UpdateMultiSelectAnswerModes();
                        HideAndClearAssignedTo_ToEmail_ToCC();
                    ...
                    if (Status != false) {
                        ...
                        $scope['CorrectiveAction'].push({ Id: 302, Name: "CAPA", ControlId: "chkCoorectiveAction", 'ColourIndex': 'red', Selected: false });
                        $scope['CorrectiveAction'].push({ Id: 303, Name: "FYI", ControlId: "chkCoorectiveAction", 'ColourIndex': '#FFBF00', Selected: true });
                        oCorrectiveAction.Set({ Id: 303, Name: "FYI", ControlId: "chkCoorectiveAction", 'ColourIndex': '#FFBF00', Selected: true });
    ...
    // To do : Need to replace with framework code (Temperory for CMFT release)
    // For supporting multi selection answer mode
    var SaveMultiSelectAnswerModes = function () {
        try {
            var oDataCaptureDAO = new DcDAO();

            var Query = "Select Id From DataCaptureEntity Where ClientGuid = '" + _oDataCaptureBO.CurrentDcClientGuid + "'";
            ...
            var DcResultId = oDataCaptureDAO.GetDcResultId_DAO(DcId, OneViewSessionStorage.Get("LoginUserId"));
                    ...
                    for (itrAnswerMode in TemplateNodeObject.AnswerMode) {
                        ...
                        if (typeof (TemplateNodeObject.AnswerMode[itrAnswerMode]) != 'function') {
                            ...
                            if (TemplateNodeObject.AnswerMode[itrAnswerMode].Type == "DCListViewControlConfig" && TemplateNodeObject.AnswerMode[itrAnswerMode].SelectionType == "MULTI") {
                                ...
                                var DataSource = $scope[TemplateNodeObject.AnswerMode[itrAnswerMode].ControlId].GetDataSource();

                                for (var i = 0; i < DataSource.length; i++) {

                                    if (DataSource[i].selected == "selected") {

                                        var _oDCResultDetailEntity = GetDcResultDetailsForMulti(DataSource[i].Answer, DataSource[i].label, TemplateNodeObject, DcId, DcResultId, Index);
            ...
            throw oOneViewExceptionHandler.Create("BO", "VictoryEquipmentSanitizerEKFC2ControllerFacadeCopy.SaveMultiSelectAnswerModes", Excep);
        }
        finally {
        }
    }
    ...
    // To do : Need to replace with framework code (Temperory for CMFT release)
    // For supporting multi selection answer mode
    var GetDcResultDetailsForMulti = function (Answer, AnswerValue, TemplateNode, DcId, DcResultId, IndexId) {
        try {
            ...
            throw oOneViewExceptionHandler.Create("BO", "VictoryEquipmentSanitizerEKFC2ControllerFacadeCopy.GetDcResultDetailsForMulti", Excep);
        }
        finally {
        }
    }
    ...
    // To do : Need to replace with framework code (Temperory for CMFT release)
    // For supporting multi selection answer mode
    var UpdateMultiSelectAnswerModes = function () {
        try {
            var oDataCaptureDAO = new DcDAO();
                    ...
                    for (itrAnswerMode in TemplateNodeObject.AnswerMode) {
                        ...
                        if (typeof (TemplateNodeObject.AnswerMode[itrAnswerMode]) != 'function') {
                            ...
                            if (TemplateNodeObject.AnswerMode[itrAnswerMode].Type == "DCListViewControlConfig" && TemplateNodeObject.AnswerMode[itrAnswerMode].SelectionType == "MULTI") {
                                ...
                                for (var i = 0; i < DataSource.length; i++) {
                                    if (DataSource[i].IndexId > Index) {
                                        Index = DataSource[i].IndexId;
                                    }
                                }

                                Index += 1;
                                for (var i = 0; i < DataSource.length; i++) {
                                    if (DataSource[i].selected == "selected" && DataSource[i].id == 0) {
                                        var _oDCResultDetailEntity = GetDcResultDetailsForMulti(DataSource[i].Answer, DataSource[i].label, TemplateNodeObject, DcId, DcResultId, Index);
                                        var _oDcResultDetailsEntityDAO = new DefaultMasterDAO("DcResultDetailsEntity");
                                        _oDcResultDetailsEntityDAO.CreateMaster(_oDCResultDetailEntity);
                                        Index += 1;
                                    }
                                    else if (DataSource[i].selected == "" && DataSource[i].id != 0) {
                                        var Query = "UPDATE DcResultDetailsEntity SET Answer = '',AnswerValue = '' WHERE Id = " + DataSource[i].id;
            ...
            throw oOneViewExceptionHandler.Create("BO", "VictoryEquipmentSanitizerEKFC2ControllerFacadeCopy.UpdateMultiSelectAnswerModes", Excep);
        }
        finally {
        }
    }
    ...
    // To do : Need to replace with framework code (Temperory for CMFT release)
    // For supporting multi selection answer mode
    var ClearMultiSelectAnswerModes = function () {
                    ...
                    for (itrAnswerMode in TemplateNodeObject.AnswerMode) {
                        ...
                        if (typeof (TemplateNodeObject.AnswerMode[itrAnswerMode]) != 'function') {
                            ...
                            if (TemplateNodeObject.AnswerMode[itrAnswerMode].Type == "DCListViewControlConfig" && TemplateNodeObject.AnswerMode[itrAnswerMode].SelectionType == "MULTI") {
                                ...
                                var _oDOM = new DOM();
                                _oDOM.RemoveInnerHtml(TemplateNodeObject.AnswerMode[itrAnswerMode].ControlId);
                                _oDOM.AddInnerHtml(TemplateNodeObject.AnswerMode[itrAnswerMode].ControlId, "");
            ...
            if (DataCaptureClientGuid != "") {
                ...
                if (DCNCMappingResult.length > 0) {
                    ...
                    if ($scope.CustomCorrectiveActionList.length > 0) {
                        ...
                        $scope['CorrectiveAction'].push({ Id: 302, Name: "CAPA", ControlId: "chkCoorectiveAction", 'ColourIndex': 'red', Selected: true });
                        $scope['CorrectiveAction'].push({ Id: 303, Name: "FYI", ControlId: "chkCoorectiveAction", 'ColourIndex': '#FFBF00', Selected: false });
                        $scope['chkCoorectiveAction'].Set({ Id: 302, Name: "CAPA", ControlId: "chkCoorectiveAction", 'ColourIndex': 'red', Selected: true });
                        ...
                        $scope['CorrectiveAction'].push({ Id: 302, Name: "CAPA", ControlId: "chkCoorectiveAction", 'ColourIndex': 'red', Selected: false });
                        $scope['CorrectiveAction'].push({ Id: 303, Name: "FYI", ControlId: "chkCoorectiveAction", 'ColourIndex': '#FFBF00', Selected: true });
                        $scope['chkCoorectiveAction'].Set({ Id: 303, Name: "FYI", ControlId: "chkCoorectiveAction", 'ColourIndex': '#FFBF00', Selected: true });
                        $scope.CustomCorrectiveAction = false;
                    }
                }
            }
    ...
    var Loadddl = function () {
        try {
            ...
            var oClassddl = new AutoCompleteUserControl({ 'Scope': $scope, 'ControlId': 'AddlClassControlId', 'DataSourceModelName': 'Class', 'DisplayElementModelName': 'NewDCModel.AddlClassControlId', 'DATEntityTypeId': DATEntityType.RCOMaster_Class, 'xlatService': xlatService, 'ToasterService': '', 'IsDynamicElementCreationEnabled': false, 'MinCharToSearch': 0, 'AttributeNodeId': 15652 });
            ...
            var oSupplierddl = new AutoCompleteUserControl({ 'Scope': $scope, 'ControlId': 'AddlSupplierControlId', 'DataSourceModelName': 'Suppliers', 'DisplayElementModelName': 'NewDCModel.AddlSupplierControlId', 'DATEntityTypeId': DATEntityType.RCOMaster_Supplier, 'xlatService': xlatService, 'ToasterService': '', 'IsDynamicElementCreationEnabled': false, 'MinCharToSearch': 0, 'AttributeNodeId': 15644 });
    ...
    var oUnitSelectedIndexChanged = function (EventArgs) {
        try {
            ...
            var oBlastChillerddl = new AutoCompleteUserControl({ 'Scope': $scope, 'ControlId': 'AddlBlastChillerControlId', 'DataSourceModelName': 'BlastChiller', 'DisplayElementModelName': 'NewDCModel.AddlBlastChillerControlId', 'DATEntityTypeId': DATEntityType.RCOMaster_BCChiller, 'xlatService': xlatService, 'ToasterService': '', 'IsDynamicElementCreationEnabled': false, 'MinCharToSearch': 0, 'AttributeNodeId': 15662 });
            ...
            if (IsFromSaveorEditClick == false) {
                SetRcoDataSourceForBand(DcPlaceId);
            }
    ...
    var oLocationSelectedIndexChanged = function (EventArgs) {
        try {
            ...
            if (EventArgs != null && EventArgs.NewValue.Id != null && EventArgs.NewValue.Id > 0) {
                ...
                var oSectionddl = new AutoCompleteUserControl({ 'Scope': $scope, 'ControlId': 'AddlSectionControlId', 'DataSourceModelName': 'Section', 'DisplayElementModelName': 'NewDCModel.AddlSectionControlId', 'DATEntityTypeId': DATEntityType.RCOMaster_Section, 'xlatService': xlatService, 'ToasterService': '', 'IsDynamicElementCreationEnabled': false, 'MinCharToSearch': 0, 'AttributeNodeId': 15634 });
    ...
    this.Temperature_NgKeyUp = function (AttributeId, ControlId, RefreshcontrolId, IsNc) {
        try {
            ...
            if (AttributeId == 15669) {
                ShowAssignedTo_ToEmail_ToCC(AttributeId,ControlId);
            }
           // alert(JSON.stringify(AssignedTo_ToEmail_ToCC));
    ...
    var ShowAssignedTo_ToEmail_ToCC = function (AttributeId, ControlId) {
        try {            
            OneViewConsole.Debug("ShowAssignedTo_ToEmail_ToCC Start", "IncidentReportFacade.ShowAssignedTo_ToEmail_ToCC");

                var DataSource = $scope[ControlId].GetDataSource();

                for (var i = 0; i < DataSource.length; i++) {

                    var RCOId = DataSource[i].Answer;
                    var AsiignedId = Band_ServiceID_1[100001][RCOId].Sequence;

                    if (Band_ServiceID_1[100001][RCOId] != undefined) {

                        if (DataSource[i].selected == "selected") {
                            $scope["Div_" + AsiignedId] = true;
                            $scope["Header_" + AsiignedId] = DataSource[i].label;

                            if ($scope.NewDCModel["txtToEmailAddress_" + AsiignedId] === "") {
                                $scope.NewDCModel["txtToEmailAddress_" + AsiignedId] = AssignedTo_ToEmail_ToCC[AsiignedId].ToEmail;                             
                            }
                            if ($scope.NewDCModel["txtCCEmailAddress_" + AsiignedId] === "") {
                                $scope.NewDCModel["txtCCEmailAddress_" + AsiignedId] = AssignedTo_ToEmail_ToCC[AsiignedId].ToCC;
                            }
                        }
                        else {
                            $scope["Div_" + AsiignedId] = false;
                            $scope["Header_" + AsiignedId] = "";
                            $scope.NewDCModel["txtToEmailAddress_" + AsiignedId] = "";
                            $scope.NewDCModel["txtCCEmailAddress_" + AsiignedId] = "";
    ...
    var LoadAnswerModes = function () {
        try {
            ...
            oRefrigeratorStatus.AnswerModes(TemplateNodes, 15639);
            ...
            oChillerAvailability.AnswerModes(TemplateNodes, 15643);
            ...
            oDayMarker.AnswerModes(TemplateNodes, 15653);
            ...
            oIsSampleNumber.AnswerModes(TemplateNodes, 15663);
            ...
            var oPriority = new AnswerModeUserControl({ 'Scope': $scope, 'ControlId': 'chkPriority', 'DataSourceModelName': 'Priority_Datasource', 'DisplayElementModelName': 'NewDCModel.chkPriority' });
            oPriority.AnswerModes(TemplateNodes, 15710);
    ...
    var LoadSource = function () {
        try {
            ...
            if ($scope["chkIsSampleNumberControlId"].GetSelectedValue() != undefined && $scope["chkIsSampleNumberControlId"].GetSelectedValue() != "") {
                if ($scope["chkIsSampleNumberControlId"].GetSelectedValue() == 1) {

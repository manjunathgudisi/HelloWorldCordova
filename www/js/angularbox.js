...
angular.module('angularbox').service('registerServ', function () {
  ...
  register = {};
  Slide = function () {
    function Slide(rel, src, title, RuleId) {
  ...
  this.add = function (rel, src, title, RuleId) {
    var slide;
    if (typeof src === 'undefined') {
      return void 0;
    }
    slide = new Slide(rel, src, title, RuleId);
    ...
    controller: [
      ...
      function ($scope, $element, $timeout, $window, $document, registerServ) {
        ...
        $scope.delete = function ($event) {           
            if (LVMultiMediaDeleteEventHandler != null) {

                var Message = 'IN-SU-LVI-015 :: Are you sure you want to Delete ?';
                var IsSuccess = oOneViewDefaultConfirmBox.Show(Message);

                if (IsSuccess == true) {
                    LVMultiMediaDeleteEventHandler(current.RuleId, current.src);
                }
                else {
                    $event.stopPropagation();
                }
                ...
                $event.stopPropagation();
                alert("This feature not enabled");
            }
        };
  ...
  function ($rootScope, registerServ) {
    return {
      ...
      link: function ($scope, $elm, $attrs) {         
        ...
        RuleId = $attrs.id;
        rel = $attrs.rel;
        id = registerServ.add(rel, $attrs.href, $attrs.title, RuleId);
